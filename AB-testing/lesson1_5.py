 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "        # Объединение датафрейма, чтобы в него вошли все заходившие во все время\n",
    "        df_full = df_sales_1.merge(df_web_1, how = 'outer', on = ['user_id', 'date']).fillna({'price':0})\n",
    "        # Выводим список user_id в ограниченный период\n",
    "        user_id = df_web_1[(df_web_1['date'] >= begin_date) & (df_web_1['date'] < end_date)].user_id.unique().tolist()\n",
    "        # Отбираем из объединенного датасета пользователей, которые заходили в обозначенный период и группируем\n",
    "        df_revenue_web = (df_full[df_full['user_id'].isin(user_id) \n",
    "                                   & (df_full['date'] >= begin_date) & (df_full['date'] < end_date)]\n",
